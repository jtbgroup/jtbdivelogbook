...
public class LogBookBrowserAction {
	...
	public LogBookBrowserAction(LogBookBrowserPanel logBookBrowserPanel,
	...
	private void createDuplicateDiveAction() {
		Action action = new DuplicateDiveAction(logBookBrowserPanel,
				logBookManagerFacade);
		actions.put(DuplicateDiveAction.class.getName(), action);
	}

	private void createDeleteDive() {
		Action action = new DeleteDivesAction(logBookBrowserPanel,
				logBookManagerFacade);
	...
	private void createExpandAction() {
		...
		action.putValue(
				...
				UIAgent.getInstance().getIcon(
						UIAgent.ICON_BTN_EXPAND_ALL_16));
