...
import be.vds.jtbdive.client.core.filters.DoubleDiveFilter;
...
public class DoubleFilterPanel extends FilterNaturePanel {
			...
			public void stateChanged(ChangeEvent e) {
				if (propagateChanges) {
					if (null != diveFilter) {
						...
						d = ((DoubleFilterInspector) (diveFilter
								.getDiveFilterType().getInspector()))
								.convertToModel(d);
						((DoubleDiveFilter) diveFilter).setCriteriaValue(d);
	...
	public void clearComponents() {
		...
		DoubleFilterInspector inspector = (DoubleFilterInspector) diveFilter
				.getDiveFilterType().getInspector();
		// double initial =
	...
	private void adaptSpinnerModel(DoubleFilterInspector inspector) {
		...
		DoubleFilterInspector di = (DoubleFilterInspector) inspector;
		double value = di.convertFromModel(((DoubleDiveFilter) diveFilter)
				.getFirstCriteria());
		double minimum = di.getMinimumValue();
		double maximum = di.getMaximumValue();
		double step = di.getStepValue();
	...
	private void setLengthUnitLabel() {
		if (diveFilter != null) {
			DoubleFilterInspector di = (DoubleFilterInspector) diveFilter
					.getDiveFilterType().getInspector();
