...
public class DiveFiltersDialog extends PromptDialog {
	...
	public DiveFiltersDialog(Frame parentDialog,
			DiverManagerFacade diverManagerFacade,
			DiveSiteManagerFacade diveSiteManagerFacade) {
		super(parentDialog, i18n.getString("filters"), i18n
				.getString("filters.define.message"), UIAgent.getInstance()
				.getBufferedImage(UIAgent.ICON_FILTER_48), null);
		this.diverManagerFacade = diverManagerFacade;
		this.diveSiteManagerFacade = diveSiteManagerFacade;
		setIconImage(UIAgent.getInstance().getBufferedImage(
				UIAgent.ICON_FILTER_16));
	}
	...
	protected Component createContentPanel() {
		...
		GridBagLayoutManager.addComponent(p, createFiltersTree(), gc, 0, 0, 1,
				1, 1, 0, GridBagConstraints.HORIZONTAL,
				GridBagConstraints.CENTER);
		GridBagLayoutManager.addComponent(p, createButtons(), gc, 1, 0, 1, 1,
				0, 0, GridBagConstraints.BOTH, GridBagConstraints.CENTER);

		GridBagLayoutManager.addComponent(p, createFilterDetails(), gc, 0, 1,
	...
	private void adaptButtons(DiveFilter df) {
		...
		if (!isRootChild)
			isRootChild = filtersTree.isRootLeaf(df);
	...
	private void displayPanelForNature(DiveFilter filter) {
		DiveFilterNature nature = filter.getDiveFilterType()
				.getDiveFilterNature();
		...
		if (filterPanel == null) {
			...
			if (filterPanel != null)
				registerFilterPanel(filterPanel, nature.getKey());
		}
	...
	private FilterNaturePanel createPanelForNature(DiveFilterNature nature) {
		...
		case INTEGER:
			...
			// case DIVE_TIME:
			// return createDiveTimeComponent();
		case AGGREGATOR:
	...
	private FilterNaturePanel createDateComponent() {
		...
		c.addPropertyChangeListener(new PropertyChangeListener() {
			@Override
			public void propertyChange(PropertyChangeEvent evt) {
				if (FilterNaturePanel.FILTER_CHANGED.equals(evt
						.getPropertyName())) {
	...
	private Component createButtons() {
		...
		GridBagLayoutManager.addComponent(p, addFilterButton(), gc, 0, 0, 1, 1,
				0, 0, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
		GridBagLayoutManager.addComponent(p, removeFilterButton(), gc, 0, 1, 1,
	...
	private Component addFilterButton() {
		addButton = new I18nButton(new AbstractAction("add") {
			...
			public void actionPerformed(ActionEvent e) {
				FilterTypeSelectionDialog d = new FilterTypeSelectionDialog(
						DiveFiltersDialog.this);
				int i = d.showDialog(300, 400);
				if (i == PromptDialog.OPTION_OK) {
					DiveFilter df = getSelectedFilter();
					filtersTree.addFilter(df, DiveFilterHelper
							.createDiveFilter(d.getFilterType()));
				}
			}
		});
	...
	//
	// private DiveFilterTypePanel createDiveTimeComponent() {
	// DiveTimeDiveFilterPanel c = new DiveTimeDiveFilterPanel();
	// c.addPropertyChangeListener(new PropertyChangeListener() {
	// @Override
	// public void propertyChange(PropertyChangeEvent evt) {
	// if (DiveFilterTypePanel.FILTER_CHANGED.equals(evt
	// .getPropertyName())) {
	// refreshLabel((DiveFilter) evt.getNewValue());
	// }
	// }
	...
	public void setDefaultFilters() {
		AggregatorDiveFilter a = new AggregatorDiveFilter(
				DiveFilterType.AGGREGATOR);
